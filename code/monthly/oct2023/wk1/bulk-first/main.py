...
data_directory = 'code/monthly/oct2023/wk1/bulk-first/processed_data'
data = []
types = []

info_df = pd.read_csv('')
info_ids = np.array(info_df['SpectraName'])
class_labels = {cls: i for i, cls in enumerate(info_df['Type'].unique())}
info_df['label'] = info_df['Type'].map(class_labels)
